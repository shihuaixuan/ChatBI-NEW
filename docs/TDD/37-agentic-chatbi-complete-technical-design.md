...
### 1.7.5 P0：补齐核心指标分析关系和真实数据集覆盖（待办）

状态：**dataset 243 已完成首批核心覆盖（2026-08-24），其他启用数据集待继续盘点和治理**。

dataset 243 本轮已补充：

- 订单平均客单价的结构化比率公式：总 GMV / 总订单数；
- 总 GMV 对总订单数、销售商品件数、总商品件数、订单平均客单价、客户当日 GMV、
  新增成交客户数的受治理分析关系；
- 客户当日 GMV 对客户当日订单数、客户当日购买件数、新增成交客户数的受治理分析关系；
- 订单日事实模型到客户交易日事实模型的一对多关系，强制
  `PRE_AGGREGATE_REQUIRED`，只允许按日期、档口预聚合后对齐；
- 核心可加指标在商家、档口、客户和交易渠道维度上的 18 个 `CONTRIBUTION` 能力；
- 已发布 Schema 的重复覆盖检查脚本，并完成契约发布和检索索引重建。

当前发布结果：`schema_version=46`、`contract_version=4`，核心分析关系覆盖 9/9，
贡献度能力覆盖 18/18。配置入口为
`backend/scripts/configure_dataset243_research_stage3_assets.py`，覆盖检查入口为
`backend/scripts/check_dataset243_semantic_coverage.py`。

当前项目已经具备指标关系、指标维度能力、维度层级、发布校验和运行时 Schema 投影结构，
当前最紧缺的不是继续增加一套关系模型，而是补齐真实数据集中的治理数据。Research 只有在
目标指标对应的分析关系已经配置、审核并随数据集发布后，才能根据语义层提供的合法范围拆分
子问题；不得让模型根据名称相似度或通用常识自行推断驱动指标和分析维度。

待办事项：

- [ ] 盘点当前启用问数的数据集、核心目标指标及其分析关系覆盖情况；
- [ ] 为核心目标指标补齐结构化公式组成关系，公式关系继续以指标公式为唯一来源，不重复配置；
- [ ] 为无法通过公式表达的指标补齐 `CERTIFIED_DRIVER` 或
  `GOVERNED_ANALYSIS_RELATION`，并明确验证方式、预期方向、时间角色、共同逻辑维度和关系路径；
- [ ] 补齐目标指标对实际分析维度的 `GROUP_BY`、`FILTER`、`DETAIL` 和
  `CONTRIBUTION` 能力，以及对应的聚合安全策略；
- [ ] 优先完成总 GMV、客户当日 GMV、总订单数、总商品件数、订单平均客单价和
  新增成交客户数之间的关系配置；
- [ ] 优先完成上述指标与交易渠道、客户、档口等维度的兼容关系配置；
- [ ] 配置完成后重新发布数据集并重建检索索引，确认 Schema 版本、关系投影和能力投影同步更新；
- [ ] 增加覆盖检查，能够明确列出目标指标缺少的分析关系、共同维度、时间角色和贡献度能力；
- [ ] 使用真实问题验证 Research 只能从已发布关系中选择分析方向，缺少关系时返回明确能力不足，
  不能静默补充资产或扩大运行范围。

验收标准：

1. 对“GMV 下降，请分析主要原因”，Research 可以从已发布关系中获得允许验证的驱动指标和
   允许下钻的维度；
2. 严格计算关系、辅助分析关系和维度分解不会混用，最终结论强度与关系类型一致；
3. 子问题使用的指标和维度全部能够追溯到当前已发布 DatasetSchema；
4. 未配置的指标关系不会被模型自行生成，系统返回明确的缺失原因；
5. 真实问数数据集的关系覆盖情况可以通过检查脚本或管理接口重复验证。
...
### 1.7.8 P2：Verified Query 和高频能力沉淀
