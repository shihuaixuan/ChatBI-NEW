...
> **✅ 阶段 8 已于 2026-08-23 执行完成**（本节以下保留勘察日原文，执行记录与
> 偏离点见 doc39 §9）。前置条件裁决：第 2–3 项（质量门槛数据、回滚观察期）
> **经运营拍板强制豁免**（用户决策"将开关改为agent吧，并进行阶段8"，风险自担）；
> 第 1 项随切流同步完成；第 4 项以 RUNNING 处置记录核销（runs 1258–1261，
> `LEGACY_REMOVAL_DISPOSED`）。§12.5 七条验收全部通过（其中"全量回归"对既有
> 失败逐一 stash 对照 clean HEAD 甄别为非本阶段回归），勾验表在 doc39 §9.6。

**勘察日原文：准备完成；删除执行未发生。** §12.2 前置条件第 1–3 项不满足（默认引擎仍为
...
（已于 2026-08-23 执行并全部通过，见 doc39 §9。）
