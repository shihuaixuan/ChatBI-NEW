...
问题主要集中在三个环节：Headless 资产缺失或定义不准确、意图结果没有形成完整查询计划、SQL 编译直接消费候选资产。
...
- 现有 ChatBI Workflow 和 Headless 测试无新增失败。
