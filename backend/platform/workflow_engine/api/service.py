...
from sqlbot_platform.workflow_engine.runtime.public_projection import (
    node_display_label,
...
class GraphApiService:
    ...
    def __init__(
        self,
        session: Session,
        extension: WorkflowApiExtension | None = None,
    ) -> None:
    ...
    def create_query(
        ...
        dataset_id = self._extension.resolve_dataset_id(
            current_user.oid,
            request.dataset_id,
        )
        request_context = {
    ...
    def create_chat_query(
        ...
        run_id = request.run_id or f"graph-{uuid4().hex}"
        try:
            prepared = self._extension.prepare_chat_query(
        ...
        except WorkflowApiRequestError as exc:
            raise HTTPException(
    ...
    def _resolve_chat_query_context(
        ...
        """调用业务扩展校验交互式 Graph 请求。"""

        try:
            return self._extension.validate_chat_query(
                workspace_id=current_user.oid,
                user_id=current_user.id,
                chat_id=chat_id,
                requested_dataset_id=request.dataset_id,
    ...
    def _build_runtime(
        ...
        try:
            return self._extension.build_runtime(
                definition_version,
                commit_events=commit_events,
                run_store=run_store,
            )
    ...
    def _trace_definition(self, definition_version: str) -> WorkflowDefinition:
        try:
            return self._extension.build_definition(definition_version)
        except WorkflowApiRequestError as exc:
            raise HTTPException(
                status_code=exc.status_code,
                detail=exc.detail,
