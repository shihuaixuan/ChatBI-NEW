...
from apps.chatbi_capabilities.semantic.compile import compile_semantic_sql
...
def test_reports_missing_headless_dataset():
    with patch(
        "apps.chatbi_capabilities.semantic.compile.resolve_dataset_by_datasource",
        return_value=None,
    ):
...
def test_compiles_with_explicit_dataset_id():
    ...
    with patch("apps.chatbi_capabilities.semantic.compile.HeadlessSchemaBuilder") as builder:
