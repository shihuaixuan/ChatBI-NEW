...
def _dataset_configs(dataset: SemanticDataset) -> list[DatasetModelConfig]:
...
def build_model_with_assets(payload: ModelCreateWithAssetsPayload, oid: int = 0) -> SemanticModelAssetBundle:
...
class SemanticSchemaBuilder:
    ...
    def build_dataset_schema(self, oid: int, dataset_id: int) -> DatasetSchema:
        if self.session is None:
            raise ValueError("SEMANTIC_SESSION_REQUIRED")
        dataset = self.session.get(SemanticDataset, dataset_id)
        if dataset is None or dataset.oid != oid or dataset.status != 1:
            raise ValueError("SEMANTIC_DATASET_NOT_FOUND")
        domain = self.session.get(SemanticDomain, dataset.domain_id)
            ...
            models = _all(
                self.session.exec(
                    select(SemanticModel).where(
                        ...
                        SemanticModel.id.in_(configured_model_ids),
            ...
            _all(
                self.session.exec(
                    select(SemanticModelField).where(
                        ...
                        SemanticModelField.model_id.in_(model_ids),
            ...
            _all(
                self.session.exec(
                    select(SemanticModelMeasure).where(
                        ...
                        SemanticModelMeasure.model_id.in_(model_ids),
        ...
        terms = _all(
            self.session.exec(
                select(SemanticTerm).where(
                    ...
                    SemanticTerm.domain_id.in_(model_domain_ids or [dataset.domain_id]),
            ...
            _all(
                self.session.exec(
                    select(SemanticModelRelation).where(
                        ...
                        SemanticModelRelation.domain_id.in_(model_domain_ids or [dataset.domain_id]),
    ...
    def _load_subject_domains(
        ...
        for domain_id in domain_ids:
            ...
            if domain is None:
                domain = self.session.get(SemanticDomain, domain_id)
...
class SemanticKnowledgeService:
    def build_words(self, schema: DatasetSchema) -> list[SemanticWord]:
    ...
    def _element_words(self, element: SchemaElement) -> list[SemanticWord]:
        raw_words = _unique([element.name, element.biz_name, *(element.alias or [])])
        if element.type == "VALUE":
            raw_words = _unique(element.alias or [])
        return [SemanticWord(text=word, element=element, word=_match_word(element, word), similarity=1.0) for word in raw_words]


class SemanticSchemaMapper:
    def __init__(self, knowledge_service: SemanticKnowledgeService | None = None):
        self.knowledge_service = knowledge_service or SemanticKnowledgeService()
...
def _selected_model_domain_ids(models: list[SemanticModel]) -> list[int]:
    domain_ids: list[int] = []
    for model in models:
...
def _runtime_subject_domains(
    models: list[SemanticModel],
    domains: list[SemanticDomain | None],
) -> list[dict[str, Any]]:
...
def _dimension_from_detail(item: dict[str, Any], oid: int, model_id: int) -> SemanticDimension:
...
def _metric_from_detail(item: dict[str, Any], oid: int, model_id: int) -> SemanticMetric:
...
def _metric_from_measure_detail(item: dict[str, Any], oid: int, model_id: int) -> SemanticMetric:
