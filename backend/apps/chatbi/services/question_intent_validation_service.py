...
class QuestionIntentValidationService:
    ...
    def __init__(
        ...
        self._validation_service = (
            validation_service or QuestionUnderstandingValidationService()
        )
    ...
    def validate(
        ...
        result = self._validation_service.validate(
            QuestionUnderstandingValidationData(
                ...
                metric_mentions=tuple(
                    str(item)
                    for item in intent.get("metric_mentions") or []
                    if str(item)
                ),
                ...
                ambiguous_slots=tuple(
                    str(item)
                    for item in intent.get("ambiguous_slots") or []
                    if str(item)
                ),
                conflict_slots=tuple(
                    str(item)
                    for item in intent.get("conflict_slots") or []
                    if str(item)
                ),
        ...
        repair_issues = [
            issue for issue in result.issues if issue.category == "repair"
        ]
    ...
    @staticmethod
    def retry_feedback(validation: dict[str, Any]) -> dict[str, Any]:
...
__all__ = ["QuestionIntentValidationService"]
