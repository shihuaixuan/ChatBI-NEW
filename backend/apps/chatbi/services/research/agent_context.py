...
def build_research_system_context(requirement: ResearchAgentRequirement) -> str:
    ...
    return (
        ...
        "</frozen-boundary>\n\n"
        "<protocol>\n"
        "1. 需要继续获取证据时提交动作；若模型已经能够直接回答，则返回纯文本。\n"
        "2. finish_research 必须单独提交，不能与数据动作或澄清动作混合。\n"
        "3. 相同参数的动作会复用已保存结果，不重复执行或消耗预算。\n"
        "4. query_semantic_data 只选择 request.operation 和业务资产；不要提交日期、"
        "时间范围、Scope、版本或底层查询结构，这些内容由服务端从冻结 Requirement"
        "补全。比较使用 compare_metrics，分解使用 breakdown，下钻使用 drilldown，"
        "贡献分析使用 contribution，驱动指标验证使用 validate_drivers。\n"
        "compute_evidence 会写入当前 Run 的 Evidence，必须单独提交；只有相互独立的"
        "query_semantic_data 或 read_evidence_rows 才能放入同一并行批次。\n"
        "5. 根据 Evidence Requirement 和已有 Evidence 决定下一步。\n"
        ...
        "Finding、Todo、Scope 和状态事件由 Runtime 维护，不要尝试修改。结束时在"
        "finish_research.findings 中只提交 statement"
        "和 evidence_ids；Runtime 根据 Evidence 生成完整 Finding。\n"
        "</protocol>\n"
