...
class GenerationHistoryProjectionService:
    ...
    def project(
        ...
        sql_messages = self._select_messages(
            data.sql_logs,
            data.regenerate_record_id,
        )
        chart_messages = self._select_messages(
            data.chart_logs,
            data.regenerate_record_id,
        )
        return GenerationHistoryProjectionResult(
            sql_history=self._project_messages(
                sql_messages,
                data.round_limit,
                SQLGenerationMessage,
            ),
            chart_history=self._project_messages(
                chart_messages,
                data.round_limit,
                ChartGenerationMessage,
            ),
        ...
        if regenerate_record_id:
            selected_log = next(
                (
                    log
                    for log in logs
                    if log.record_id == regenerate_record_id
                ),
                None,
            )
    ...
    def _project_messages(
        ...
        for message in cls._last_conversation_rounds(messages, round_limit):
...
__all__ = ["GenerationHistoryProjectionService"]
