...
class SlotClarificationNeededCondition:
    ...
    def evaluate(
        ...
        raw_temporal_plan = (
            intent.get("temporal_plan") if isinstance(intent, dict) else None
        )
        temporal_plan: dict[str, object] = (
            raw_temporal_plan if isinstance(raw_temporal_plan, dict) else {}
        )
        if temporal_plan.get("status") in {"clarification_required", "unsupported"}:
            return ConditionDecision(
                matched=True,
                reason_code="TEMPORAL_CLARIFICATION_NEEDED",
                reason_summary="需要用户确认时间范围",
            )
        slot_response = read_interaction_response(
            variables, "ask_slot_clarification", "slot_response"
        )
        ...
        raw_validation = intent.get("validation") if isinstance(intent, dict) else None
        ...
        return ConditionDecision(
            matched=matched,
            reason_code="SLOT_CLARIFICATION_NEEDED"
            if matched
            else "SLOT_CLARIFICATION_NOT_NEEDED",
            reason_summary="需要用户补充槽位信息"
            if matched
            else "无需用户补充槽位信息",
        )


class TemporalSlotAnsweredCondition:
    """时间澄清回答必须重新进入时间理解，不能直接开始知识检索。"""
    ...
    def evaluate(
        ...
        has_confirmation = any(
            ...
            for value in (
                response.get("temporal_confirmation"),
                response.get("time_range"),
                response.get("text"),
            )
        )
        matched = bool(
            temporal_plan.get("status") in {"clarification_required", "unsupported"}
            and response.get("skipped") is not True
            and has_confirmation
        )
        return ConditionDecision(
            matched=matched,
            reason_code=(
                "TEMPORAL_SLOT_ANSWERED" if matched else "TEMPORAL_SLOT_NOT_ANSWERED"
            ),
            reason_summary=(
                "用户已确认时间范围" if matched else "当前回答不是时间范围确认"
            ),
        )
...
class KnowledgeMetricAmbiguousCondition:
    ...
    def evaluate(
        ...
        knowledge = context.variables.get("knowledge", {})
        matched = (
            bool(knowledge.get("ambiguities"))
            or knowledge.get("status") == "metric_ambiguous"
        )
        return ConditionDecision(
            matched=matched,
            reason_code="KNOWLEDGE_METRIC_AMBIGUOUS"
            if matched
            else "KNOWLEDGE_METRIC_CLEAR",
...
class KnowledgeCrossModelCondition:
    ...
    def evaluate(
        ...
        matched = (
            knowledge.get("status") == "cross_model"
            and isinstance(plans, list)
            and len(plans) > 1
        )
        return ConditionDecision(
            matched=matched,
            reason_code="KNOWLEDGE_CROSS_MODEL"
            if matched
            else "KNOWLEDGE_SINGLE_MODEL",
            reason_summary="指标来自不同模型，需要确认拆分查询"
            if matched
            else "指标属于单一模型",
        )
...
class InteractionAnsweredCondition:
    ...
    def evaluate(
        ...
        return ConditionDecision(
            matched=matched,
            reason_code="INTERACTION_ANSWERED"
            if matched
            else "INTERACTION_NOT_ANSWERED",
...
class InteractionSkippedCondition:
    ...
    def evaluate(
        ...
        matched = any(
            isinstance(value, dict) and value.get("skipped") is True
...
class CrossModelSplitRequestedCondition:
    ...
    def evaluate(
        ...
        response = read_interaction_response(
            context.variables, "ask_cross_model_split", "cross_model_response"
        )
        action = (
            response.get("cross_model_action") if isinstance(response, dict) else None
        )
        matched = action == "split"
        return ConditionDecision(
            matched=matched,
            reason_code="CROSS_MODEL_SPLIT_REQUESTED"
            if matched
            else "CROSS_MODEL_SPLIT_NOT_REQUESTED",
...
class SqlExecutionSucceededCondition:
    ...
    def evaluate(
        ...
        return ConditionDecision(
            matched=matched,
            reason_code="SQL_EXECUTION_SUCCEEDED"
            if matched
            else "SQL_EXECUTION_NOT_SUCCEEDED",
...
class SqlErrorRetryableCondition:
    ...
    def evaluate(
        ...
        repair_plan = (
            sql_error.get("repair_plan")
            if isinstance(sql_error.get("repair_plan"), dict)
            else {}
        )
        matched = (
            bool(sql_error.get("retryable"))
            and repair_plan.get("action") == "regenerate_sql"
        )
        return ConditionDecision(
            matched=matched,
            reason_code="SQL_ERROR_RETRYABLE" if matched else "SQL_ERROR_NOT_RETRYABLE",
            reason_summary="SQL 错误可通过重新生成 SQL 重试"
            if matched
            else "SQL 错误不可自动重试",
        )
...
class InteractionResponseAnsweredCondition:
    ...
    def evaluate(
        ...
        value = read_interaction_response(
            context.variables, self._node_name, self._legacy_key
        )
        ...
        return ConditionDecision(
            ...
            reason_summary=f"用户已回答{self._label}"
            if matched
            else f"用户尚未回答{self._label}",
        )
...
class InteractionResponseSkippedCondition:
    ...
    def evaluate(
        ...
        return ConditionDecision(
            matched=matched,
            reason_code="INTERACTION_SKIPPED" if matched else "INTERACTION_NOT_SKIPPED",
            reason_summary=f"用户跳过了{self._label}"
            if matched
            else f"用户未跳过{self._label}",
        )
...
class ClarificationRoundGate:
    ...
    def __init__(
        self, inner, ask_node: str, max_rounds: int = 2, exhausted: bool = False
    ) -> None:
        self._inner = inner
    ...
    def evaluate(
        ...
        if self._exhausted:
            ...
            return ConditionDecision(
                matched=matched,
                reason_code="CLARIFICATION_ROUNDS_EXHAUSTED"
                if matched
                else inner_decision.reason_code,
                reason_summary=(
        ...
        return ConditionDecision(
            matched=matched,
            reason_code=inner_decision.reason_code
            if matched
            else (
                "CLARIFICATION_ROUNDS_EXHAUSTED"
                if inner_decision.matched
                else inner_decision.reason_code
            ),
...
def register_chatbi_conditions(registry: ConditionRegistry) -> None:
    ...
    registry.register(
        "interaction.slot.temporal_answered", TemporalSlotAnsweredCondition()
    )
    ...
    registry.register(
        "cross_model.split_requested", CrossModelSplitRequestedCondition()
    )
    ...
    for spec in CHATBI_V1_INTERACTION_SPECS.values():
        ...
        registry.register(
            ...
            InteractionResponseAnsweredCondition(
                spec.node_name, spec.legacy_key, label
            ),
        )
        registry.register(
    ...
    for name, (inner, ask_node) in clarification_gates.items():
        ...
        registry.register(
            ...
            ClarificationRoundGate(
                inner, ask_node, max_rounds=max_rounds, exhausted=False
            ),
        )
        registry.register(
            f"clarify.{name}.exhausted",
            ClarificationRoundGate(
                inner, ask_node, max_rounds=max_rounds, exhausted=True
            ),
        )
