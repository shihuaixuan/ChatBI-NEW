...
| 模块 | 文件 | 作用 |
| --- | --- | --- |
| 图 API | `backend/platform/workflow_engine/api/service.py` | 创建 Graph Run，并同步执行当前最小图 |
| 图运行时 | `backend/platform/workflow_engine/runtime/graph_runtime.py` | 控制 Run 生命周期、循环推进节点、失败/暂停/成功 |
| 节点调度 | `backend/platform/workflow_engine/runtime/scheduler.py` | 根据 `node.handler` 找到 handler 并执行 |
| 条件路由 | `backend/platform/workflow_engine/runtime/router.py` | 按边优先级和条件判断下一个节点 |
| 上下文写入 | `backend/platform/workflow_engine/runtime/context_patcher.py` | 将节点输出 patch 到 `WorkflowContext` |
