...
from apps.headless.storage_sync import unique_texts
...
def build_metric_relations(metric: HeadlessMetric) -> list[HeadlessAssetRelation]:
...
def build_dimension_relations(dimension: HeadlessDimension) -> list[HeadlessAssetRelation]:
...
def build_dimension_value_relations(value: HeadlessDimensionValue) -> list[HeadlessAssetRelation]:
    if value.id is None:
        return []
    return [_relation(value.oid, "VALUE", value.id, "BELONGS_TO", "DIMENSION", value.dimension_id)]


def build_term_relations(term: HeadlessTerm) -> list[HeadlessAssetRelation]:
    if term.id is None:
        return []
    relations = [
...
def build_dataset_asset_relations(assets: list[HeadlessDataSetAsset]) -> list[HeadlessAssetRelation]:
    relations: list[HeadlessAssetRelation] = []
    for asset in assets:
