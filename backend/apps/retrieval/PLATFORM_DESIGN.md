...
6. `capabilities.semantic.retrieval` 仍反向导入 Workflow 适配器，说明检索核心尚未形成独立能力边界。
...
便于结果复现和问题定位。候选结果返回后进入语义解析模型，不进入执行规划。
...
flowchart LR
    ...
    R --> CAND["候选结果"]
    CAND --> P["语义解析模型"]
    P --> OUT["SemanticParseOutput"]
```

- Graph 和 Agent 都用确定性节点调用候选检索服务，再把同一份候选结果交给语义解析模型。
- Agent 工具继续采用“无自由文本参数”，从共享 state 读取已经确认的问题理解，防止 Agent 私自改变检索意图。
- 两个入口只负责传递运行上下文，不注入不同的向量会话或检索器实现。
- 当前阶段返回候选分组和 `SemanticParseOutput`，不产生执行白名单和查询计划。
