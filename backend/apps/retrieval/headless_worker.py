...
from apps.retrieval.headless_indexing import build_headless_index_profile
from apps.retrieval.indexing import IndexJobRunResult, RetrievalIndexingService
from common.core.db import engine

logger = logging.getLogger(__name__)
_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="headless-index")


def process_headless_index_jobs(
...
def submit_pending_headless_index_jobs() -> Future[list[IndexJobRunResult]]:
    """应用启动时异步恢复 durable pending 任务，避免阻塞服务就绪。"""

    future = _executor.submit(process_headless_index_jobs)
...
def _log_worker_failure(future: Future[list[IndexJobRunResult]]) -> None:
    ...
    if error is not None:
        logger.error(
            "Headless 统一索引 worker 执行失败",
            exc_info=(type(error), error, error.__traceback__),
        )


__all__ = ["process_headless_index_jobs", "submit_pending_headless_index_jobs"]
