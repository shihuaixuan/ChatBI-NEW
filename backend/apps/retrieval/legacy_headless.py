# mypy: disable-error-code="arg-type,list-item,union-attr,var-annotated"
...
from apps.retrieval.headless import DenseChannelTracker


class HeadlessRetrievalConfig(Protocol):
    """旧检索算法实际依赖的最小配置边界。"""

    @property
    def candidate_accept_score(self) -> float: ...

    @property
    def candidate_low_confidence_score(self) -> float: ...

    @property
    def candidate_ambiguity_gap(self) -> float: ...


@dataclass(frozen=True, slots=True)
class DefaultHeadlessRetrievalConfig:
    """脱离 Workflow 后的默认候选门控参数。"""
...
class HeadlessKnowledgeAdapter:
    """基于 Headless DataSetSchema 的旧检索核心。"""
    ...
    def __init__(
        ...
        config: HeadlessRetrievalConfig | None = None,
    ) -> None:
        effective_config: HeadlessRetrievalConfig = config or DefaultHeadlessRetrievalConfig()
        ...
        self._candidate_gate = candidate_gate or CandidateGate(
            accept_score=effective_config.candidate_accept_score,
            low_confidence_score=effective_config.candidate_low_confidence_score,
            ambiguity_gap=effective_config.candidate_ambiguity_gap,
        )
    ...
    def retrieve_semantic(
        ...
        *,
        ...
        candidate_groups = self._retrieve_candidate_groups(question, intent, retrieval_schema, tenant_id)
...
class HeadlessDocumentRetriever:
    ...
    def __init__(
        ...
        top_k: int = 20,
        metric_embedding_session: Any = None,
        metric_embedding_provider: EmbeddingProvider | None = None,
        dense_tracker: DenseChannelTracker | None = None,
        ...
        self._dense_tracker = dense_tracker
    ...
    def _merge_metric_embedding_candidates(
        ...
        try:
            ...
            self._handle_dense_error(exc, started)
            return
        except ProgrammingError as exc:
            index_error = RetrievalIndexUnavailableError(
                "指标向量索引不可用",
                details={"reason_code": "EMBEDDING_INDEX_UNAVAILABLE"},
            )
            if self._dense_tracker is not None:
                self._dense_tracker.record_error(
                    index_error,
                    latency_ms=(perf_counter() - started) * 1000,
                )
            if not self._allow_lexical_fallback:
                raise index_error from exc
            return
        except SQLAlchemyError as exc:
            query_error = RetrievalQueryError(
                "指标向量索引查询失败",
                details={"reason_code": "EMBEDDING_INDEX_QUERY_FAILED"},
            )
            if self._dense_tracker is not None:
                self._dense_tracker.record_error(
                    query_error,
        ...
        if self._dense_tracker is not None:
            self._dense_tracker.record_success(
                latency_ms=(perf_counter() - started) * 1000,
                candidate_count=len(matches),
            )

    def _handle_dense_error(self, error: RetrievalError, started: float) -> None:
        if self._dense_tracker is not None:
            self._dense_tracker.record_error(
                error,
                latency_ms=(perf_counter() - started) * 1000,
