...
def _resolve_dataset_model(session, oid: int, dataset: HeadlessDataSet) -> HeadlessModel | None:
    if dataset.default_model_id:
        default_model = session.get(HeadlessModel, dataset.default_model_id)
        if (
    ...
    statement = (
        select(HeadlessModel)
        .join(HeadlessDataSetModelConfig, HeadlessDataSetModelConfig.model_id == HeadlessModel.id)
        .where(
