...
const flow = computed(() => buildAgentFlow(trace.value, liveEvents.value, props.runtimeLoading))
const budget = computed(() => trace.value?.budget || {})

async function loadTrace(force = false) {
  if (!currentRecordId.value) return
  // 实时执行完全依赖 SSE；trace 只服务历史恢复与用户主动刷新。
  if (!force && (props.runtimeLoading || props.record?.status === 'waiting_user')) return
  traceLoading.value = true
  try {
    trace.value = await agentQuestionApi.trace(currentRecordId.value)
  } catch (error) {
    console.warn(error)
  } finally {
...
onMounted(loadTrace)
watch(currentRecordId, () => loadTrace())
</script>

<template>
  <section v-if="flow.steps.length || runtimeLoading || traceLoading" class="agent-trace">
    <button type="button" class="trace-head" @click="expanded = !expanded">
    ...
    <div v-show="expanded" class="flow-body">
      ...
      <div class="trace-footer">
        <span v-if="trace?.run_id">Run #{{ trace.run_id }}</span>
        <span v-if="flow.failedCount">{{ flow.failedCount }} 次失败后恢复</span>
        <button type="button" class="refresh-btn" :disabled="traceLoading" @click="refreshTrace">
          <el-icon :class="{ spinning: traceLoading }"><RefreshRight /></el-icon>
          刷新
        </button>
      </div>
    </div>
