...
import AgentClarificationCard from '@/views/chat/clarification/AgentClarificationCard.vue'
...
function handleEvent(data: any, currentRecord: ChatRecord) {
  ...
  switch (data.type) {
    ...
    case 'thinking':
      ...
      break
    case 'tool-called':
      currentTool.value = payload.tool_name || ''
      break
    case 'tool-result':
      currentTool.value = ''
...
async function sendMessage() {
  ...
  try {
    const response = await agentQuestionApi.add(
      {
...
async function submitClarification(answer: AgentClarificationAnswer) {
  ...
  const controller = new AbortController()
  try {
    const response = await agentQuestionApi.clarification(currentRecord.id, answer, controller)
    await consumeStream(response, currentRecord, controller)
  } finally {
    _loading.value = false
  }
...
<template>
  <BaseAnswer v-if="message" :message="message" :reasoning-name="reasoningName" :loading="_loading">
    <div v-if="_loading && (thinkingText || currentTool)" class="agent-status">
      <span v-if="currentTool" class="agent-tool">正在调用 {{ currentTool }}…</span>
      <span v-else class="agent-thinking">{{ thinkingText }}</span>
    </div>
    <ChartBlock
    ...
    />
    <AgentTrace :record-id="message.record?.id" :refresh-key="traceRefreshKey" />
    <slot></slot>
    <template #tool>
      <slot name="tool"></slot>
    </template>
